/* THE ANCHOR, FROM BOTH LEGS ON THE SAME CLIPS — analysis for scripts/anchor-crossed.js.
 *
 ...
 *
 * USAGE: node scripts/anchor-crossed-analyse.mjs
 */
import fs from 'fs';
...
const data = JSON.parse(fs.readFileSync('data/anchor-crossed.json', 'utf8'));
console.log(`\n  ${data.films.length} films, levels ${data.levels.join('/')}, generations ${data.gens.join('/')}\n`);
console.log(`  ${'film'.padEnd(34)} ${'dP/dlogbpp'.padStart(11)} ${'dP/dgen'.padStart(9)} ${'anchor%'.padStart(9)}  status`);
...
for (const f of data.films) {
  ...
  const sB = fit(B.map((r) => r.gen), B.map((r) => pOf(r)));
...
console.log(`    published BD-rate              5 - 40%`);
console.log(`    library gap / ladder slope     69.7%   (11.30, the transfer-assumption estimate)`);
console.log(`    THIS, no transfer assumption   ${pooled.toFixed(1)}%`);
const m = pooled;
if (m >= 5 && m <= 40) {
  console.log('\n  VERDICT: the transfer assumption WAS inflating it. Adopt this figure; the lab default');
  console.log('  of 15% is corroborated and 11.30\'s 69.7% should be marked as superseded.');
} else if (m > 40 && m <= 100) {
  console.log('\n  VERDICT: our encoder genuinely loses more per generation than published BD-rate figures');
  console.log('  suggest — which is plausible, since BD-rate compares ENCODERS and this measures a');
  console.log('  re-encode of already-damaged input. Raise the lab default and say why.');
} else {
  console.log('\n  VERDICT: outside both standing answers. Do not adopt. Report the number, keep the');
  console.log('  slider, and treat the anchor as unresolved.');
}
console.log('');

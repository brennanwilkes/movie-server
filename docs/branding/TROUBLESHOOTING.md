...
- **Jellyfin destroys & rebuilds the drawer nav on every open/close.** Anything we inject (Top 100 / Search / Settings entries, inline styles, reorder) MUST be re-applied every `scan()` and be **idempotent** (no-op when already present). A one-shot/boolean guard makes injected entries vanish after the first rebuild.
